...
func AudioFormFiller(c *gin.Context) {
	...
	promptTemplate := fmt.Sprintf("Extract the following information from the audio file:\n%s\n\nPlease analyze the audio and provide the details in json in name:value format only where the 'name' attribute is exactly the name of the form field provided in above json in camelcase. The extracted 'value' attribute should be in English Only, regardless of the language spoken in the audio.",
		fields)
